...
// In-place binary op: lhs[i] = OP(lhs[i], rhs[i]).  No separate output pointer.
...
// Fused binary+relu: out[i] = max(0, lhs[i] + rhs[i]).
#define BINARY_FUSED_ADD_RELU(TYPENAME, FN_NAME) \
extern "C" __global__ void FN_NAME( \
    const size_t numel, \
    const size_t num_dims, \
    const size_t *dims_and_strides, \
    ...
    bool lhs_cont = dims_and_strides == nullptr || is_contiguous(num_dims, dims, lhs_strides); \
    bool rhs_cont = dims_and_strides == nullptr || is_contiguous(num_dims, dims, rhs_strides); \
    TYPENAME zero = static_cast<TYPENAME>(0); \
    if (lhs_cont && rhs_cont) { \
        for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel; i += blockDim.x * gridDim.x) { \
            TYPENAME s = lhs[i] + rhs[i]; \
            out[i] = s > zero ? s : zero; \
        } \
    } else if (lhs_cont) { \
        for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel; i += blockDim.x * gridDim.x) { \
        ...
        for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel; i += blockDim.x * gridDim.x) { \
            ...
            TYPENAME s = lhs[lhs_i] + rhs[rhs_i]; \
            out[i] = s > zero ? s : zero; \
        } \
    } \
} \
